<template>
  <div>
    <div
      v-if="no_polls_answered === true"
    >To see results here, you need to vote in your first active poll.</div>
    ...
    <div style="display:flex;">
      <div class="mb-10">
        <ul style="line-height: 1.8;">
          <li v-for="poll in userAnsweredPolls" :key="poll.pollId" class="pt-5">
            <input type="radio" v-bind:id="poll.pollId" v-bind:value="poll.pollId" v-model="showAnswersFor" />
            <label :for="poll.pollId" class="pl-1">
              {{mapping[poll.pollId]}}
              <br />
              You voted: {{answer_mappings[poll.pollId][poll.answerId]}}
            </label>
          </li>
        </ul>
      </div>
...
export default {
    ...
    data() {
        return {
            ...
            mapping: {
                ...
                "1f334019-14a7-4b38-a85e-05c2b9e82906": "Andrew Yang - Mayor NYC"
            },
            answer_mappings: {
    ...
    methods: {
        getUserResults() {
            ...
            return new Promise((resolve, reject) => {
                axios.get(`https://ccusbackendcontituentpolling.azurewebsites.net/api/v1/${userId}/polls`, {
                    headers: {
    ...
    computed: {
        getPollResults() {
            axios.get(`https://ccusbackendcontituentpolling.azurewebsites.net/api/v1/results/${this.showAnswersFor}`, {
                headers: {
                    "Authorization": "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsImtpZCI6Ilg1ZVhrNHh5b2pORnVtMWtsMll0djhkbE5QNC1jNTdkTzZRR1RWQndhTmsifQ..nxKEqvYPuhgPGdLCW0KafMjr5MlgtuYZhUlyZjL_7_J9wOmyURvzondVSDIThkYtbw-hk5zX9hzgkNj-fZkU34gV0ZusFavA3FKCR2zMeUnnsWKR8M4MUpfHAll2nU3kj8HfztVl11OUiJJk5wm5GfJ8IrwuqS0yLiCpC-geYhb2Pu7rj1DfDmrxRfz-bLaZ2hQra4O-C8ukI49UaorbBgh5L6_LfuJxZbOdkjEGFWysHCZ2hCkK-Gbo1FzCR3bsY822tOjtzDXSORerxcS38Lc-uOoV7dQ-pJSJkrIPay6YODJSafVfa42t3qUpNWLwhE3pGKxEfeOEN9yQkULpAA"
                }
            })
